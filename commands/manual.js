...
const MANUALS = {
  quick: [
    ...
    '',
    '### C. 按钮报名',
    '1. `/attendance create title:<标题> [description] [select_role]`  - 发一条按钮报名帖；select_role:true 时须先选择黑色沙漠职业。',
  ...
  attendance: [
    ...
    '',
    '### 四个命令',
    '- `/attendance create title:<标题> [description] [select_role]`：创建报名帖；`select_role:true` 时需要选择黑色沙漠职业。',
    ...
    '',
    '### 最短流程',
    '1. `/attendance create title:周六黑沙团 description:请参加的人点击报名 select_role:true`',
    '2. 普通报名帖点击「报名」或「取消报名」；启用 select_role 时，直接从报名帖的职业菜单选择。',
